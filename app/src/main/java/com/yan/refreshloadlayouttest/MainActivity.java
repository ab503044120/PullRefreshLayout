...
import com.yan.pullrefreshlayout.PullRefreshLayoutTwink;
...
public class MainActivity extends AppCompatActivity {
    ...
    private List<String> mDatas;
    private PullRefreshLayoutTwink refreshLayout;
    ...
    private void initRefreshLayout() {
        refreshLayout = (PullRefreshLayoutTwink) findViewById(R.id.refreshLayout);
        refreshLayout.setLoadMoreEnable(true);
//        refreshLayout.setDuringAdjustValue(10f);// 动画执行时间调节，越大动画执行越慢
        // 刷新或加载完成后回复动画执行时间，为-1时，根据setDuringAdjustValue（）方法实现
//        refreshLayout.setRefreshBackTime(300);
...
//        refreshLayout.setUseAsTwinkLayout(true);
        ...
        refreshLayout.setOnRefreshListener(new PullRefreshLayoutTwink.OnRefreshListener() {
            @Override
            public void onRefresh() {
                refreshLayout.postDelayed(new Runnable() {
                    @Override
